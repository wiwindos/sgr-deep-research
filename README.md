...
# 4. Запустите агента
python simple_dsgr_researcher.py
```
...
def detect_language(text: str) -> str:
    """Простое определение языка на основе соотношения кириллических символов"""
...
### Адаптация системных промптов
- Русскоязычные промпты для русских запросов
- Англоязычные промпты для английских запросов
- Поисковые запросы на том же языке, что и запрос пользователя
- Отчеты полностью на языке пользователя
...
- `RUSSIAN_THRESHOLD` - Порог для определения русского языка (0.3)
...
### Файлы проекта
- `simple_dsgr_researcher.py` - Основная версия SGR Research Agent
- `sgr-deep-research.py` - Расширенная версия с дополнительными возможностями  
...
## Technical Analysis
Детальный технический анализ с цитатами [1], [2], [3]...
